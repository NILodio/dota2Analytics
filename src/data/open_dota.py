...
class OpenDotaAPI(object):
    ...
    def get_model_features_from_input(self, input_list):
        ...
        Args:
                input_list (list): contains the following integers
                from the front-end
                ['team1_hero1', 'team1_hero2',
                'team1_hero3', 'team1_hero4',
                'team1_hero5',
                'team2_hero1',
                'team2_hero2',
                'team2_hero3',
    ...
    def get_hero_wins(self, team_heroes_ids, team_heroes_info):
        ...
        for hero_id in team_heroes_ids:
            ...
            for hero_info in team_heroes_info:
                ...
                if (
                    "hero_id" in hero_info
                    and "wins" in hero_info
                    and hero_info["hero_id"] == hero_id
                ):
        ...
        if len(team_heroes_wins) != 5:
            raise Exception(
                "Error trying to get one of the hero wins from \
                one of the teams, using OpenDotaAPI."
            )
        ...
        while remaining_matches > 0:
            ...
            for match in response:
                ...
                r_team_1_info = (
                    ...
                    else self.get_team_info(team1_id)
                )
                r_team_2_info = (
                    teams[team2_id]
                    if team2_id in teams
                    else self.get_team_info(team2_id)
                )
                ...
                r_team_1_players = (
                    players[team1_id]
                    if team1_id in players
                    else self.get_team_players_info(team1_id)
                )
                r_team_2_players = (
                    players[team2_id]
                    if team2_id in players
                    else self.get_team_players_info(team2_id)
                )
                ...
                r_team_1_heroes = (
                    heroes[team1_id]
                    if team1_id in heroes
                    else self.get_team_heroes_info(team1_id)
                )
                r_team_2_heroes = (
                    heroes[team2_id]
                    if team2_id in heroes
                    else self.get_team_heroes_info(team2_id)
                )
                heroes[team1_id] = r_team_1_heroes
                heroes[team2_id] = r_team_2_heroes

                match_dict = self.set_match_data(
                    match,
                    r_match,
                    r_team_1_info,
                    r_team_2_info,
            ...
            if len(matches) % checkpoint_size == 0:
                ...
                file_name = (
                    f"./{self.output_filepath}/matches_checkpoint_{len(matches)}.csv"
                )
                print(file_name)
    ...
    def set_match_data(
        ...
        match = self.add_players_data(
            match, match_data["players"], team1players_data, "1"
        )
        # add team 2 players columns
        match = self.add_players_data(
            match, match_data["players"], team2players_data, "2"
        )
    ...
    def add_heroes_data(self, match, team_heroes_, team_number):
        ...
        for i in range(1, 6):
            ...
            match[column_name + "_gamesPlayed"] = (
                team_heroes[hero_id]["games_played"] if hero_id in team_heroes else None
            )
            match[column_name + "_wins"] = (
                team_heroes[hero_id]["wins"] if hero_id in team_heroes else None
            )
    ...
    def add_players_data(self, match, match_players, team_players_, team_number):
        ...
        if team_players_ is not None:
            for player in team_players_:
                team_players[player["account_id"]] = {
        ...
        for player in match_players:
            ...
            if player["account_id"] in team_players:
                match[column_name + "_gamesPlayed"] = team_players[
                    player["account_id"]
                ]["games_played"]
                match[column_name + "_wins"] = team_players[player["account_id"]][
                    "wins"
                ]
            else:
                match[column_name + "_gamesPlayed"] = None
    ...
    def get_pro_match_info(self, match_id):
        ...
        for match in response:
            pro_match_dict = {
                ...
                "radiant_team_id": (
                    match["radiant_team_id"] if "radiant_team_id" in match else None
                ),
                "radiant_name": (
                    match["radiant_name"] if "radiant_name" in match else None
                ),
                "dire_team_id": (
                    match["dire_team_id"] if "dire_team_id" in match else None
                ),
    ...
    def get_match_info(self, match_id):
        ...
        if "players" in response:
            for player in response["players"]:
                ...
                acc_slot = (
                    None if "player_slot" not in player else player["player_slot"]
                )
        ...
        if not response:
            ...
            return {
                ...
                "picks_bans": (
                    response["picks_bans"] if "picks_bans" in response else None
                ),
    ...
    def get_team_info(self, team_id):
        ...
        if not response:
            ...
            return {
                ...
                "last_match_time": (
                    response["last_match_time"]
                    if "last_match_time" in response
                    else None
                ),
            }
